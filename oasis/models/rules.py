...
class Rules(Model):
    def __init__(self, md_instance, store, model, data_source, slack_channel,
        ...
        super(Rules, self).__init__(
            ...
            RulesConfig(
                store.get_model_template(RULES_MODEL_NAME),
    ...
    def run_action(self, metric, val, config, rules):
        logger.info("{log_prefix}[metric:{metric}] start ot run".format(
            log_prefix=self.log_prefix, metric=metric))
    ...
    def on_error(self, metric, rule):
        logger.error("{log_prefix}[metric:{metric}] not match rule: {rule}"
                     .format(
                         log_prefix=self.log_prefix, metric=metric, rule=rule))
        self.send_to_slack(
            "{log_prefix}[model:{model}][metric:{metric}] not match rule: {rule}"
            .format(
                log_prefix=self.log_prefix,
                model=self.name,
                metric=metric,
