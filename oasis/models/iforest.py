...
from oasis.models.model import (Model, Config, MODEL_RUNNING, MODEL_FINISH,
                                MODEL_STOP)
...
class IForest(Model):
    def __init__(self, md_instance, store, model, data_source, slack_channel,
        ...
        super(IForest, self).__init__(
            ...
            IForestConfig(
                store.get_model_template(IFOREST_MODEL_NAME),
    ...
    def train(self, metric, query_expr, config):
        ...
        for index in range(0, self.cfg.model["train_count"], 1):
            ...
            if index % 10 == 0:
                ...
                self.df[metric] = self.df[metric].append(
                    df_one, ignore_index=True)
    ...
    def train_task(self, metric, query, config):
        ...
        if len(data_set) > 0:
            ...
            logger.info(
                "{log_prefix}[metric:{metric}] append data to train df:{df_one}"
                .format(
                    log_prefix=self.log_prefix, metric=metric, df_one=df_one))
    ...
    def on_error(self, metric, predict_data):
        ...
        self.send_to_slack(
            ...
            "predict data:{predict_data}".format(
                log_prefix=self.log_prefix,
                model=self.name,
                metric=metric,
                predict_data=predict_data), self.slack_channel)
